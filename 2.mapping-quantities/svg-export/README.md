...
Look in the HTML for a block of `<script>` tags to see where everything else is being loaded and note that _your_ `.js` file is listed **last**. This is essential since your code ‘depends’ on all those other files and they will be loaded in the order in which they’re listed.
